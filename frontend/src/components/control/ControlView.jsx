    ...
    <div id="game-control-view" className="space-y-4 max-w-5xl mx-auto font-sans select-none pb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-7 bg-white border border-slate-200 rounded-2xl p-3.5 sm:p-4 shadow-xs space-y-3 flex flex-col justify-between">
              ...
              {cameraActive && (
                <>
                  <button
                    onClick={flipCamera}
                    title="Flip camera"
                    className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-300 transition cursor-pointer flex items-center gap-1"
                  >
                  ...
                  <button
                    ...
                    className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 transition cursor-pointer"
                  >
                    Stop
                  </button>
                </>
          ...
          {cameraError && (
            <div className="p-2.5 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 text-[11px] font-semibold flex items-center justify-between gap-2">
              <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className="px-2 py-0.5 rounded-lg bg-amber-600 text-white text-[10px] font-bold hover:bg-amber-700 cursor-pointer shrink-0"
              >
                Retry
              </button>
            </div>
          ...
          <div className="relative aspect-video w-full rounded-xl bg-slate-950 border border-slate-800 overflow-hidden flex items-center justify-center shadow-inner">
            ...
            {/* 3. Standby State with 1-click connect buttons */}
            {!activeMediaStream && !liveMobileFrame?.image && (
              <div className="absolute inset-0 bg-slate-950 flex flex-col items-center justify-center p-4 text-center space-y-2 z-0">
                <div className="w-10 h-10 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400">
                  <Camera className="w-5 h-5" />
                </div>
                <div>
                  <div className="text-xs font-black text-slate-200">LIVE CAMERA NOT CONNECTED</div>
                  <p className="text-[10px] text-slate-400 max-w-xs mt-0.5">
                    Connect your laptop webcam or phone camera to view live video while steering.
                  </p>
                </div>
                <div className="flex flex-wrap items-center justify-center gap-1.5 pt-1">
                  ...
                  >
                    ...
                    <span>{localCamStarting ? 'Connecting...' : '💻 Connect Laptop Cam'}</span>
                  </button>
                  <button
            ...
            <div className="absolute inset-0 flex flex-col justify-between p-3 select-none pointer-events-none z-10">
              {/* Top HUD */}
              <div className="flex justify-between items-start text-[10px] text-slate-300 font-mono">
                <div className="bg-black/70 backdrop-blur-md px-2 py-0.5 rounded border border-white/10 text-emerald-400 font-bold flex items-center gap-1">
                  <span className={`w-1.5 h-1.5 rounded-full ${activeMediaStream || liveMobileFrame ? 'bg-emerald-400 animate-pulse' : 'bg-slate-400'}`} />
                  <span>{activeMediaStream ? '🟢 LAPTOP/MOBILE LIVE' : liveMobileFrame ? '🟢 PHONE NODE LIVE' : '⚪ CAMERA STANDBY'}</span>
                </div>
